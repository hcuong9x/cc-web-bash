#!/bin/bash
# =============================================
# Optimize Webinoly Server 2GB RAM - Cân bằng 2026
...
MYSQL_CNF="/etc/mysql/mariadb.conf.d/99-webinoly-2gb.cnf"
...
SYSCTL_CNF="/etc/sysctl.d/99-webinoly-2gb.conf"
...
echo "=== Starting Optimization for 2GB RAM Server ==="
...
set_ini_value "$PHP_POOL" "pm.max_children" "12"
set_ini_value "$PHP_POOL" "pm.start_servers" "4"
set_ini_value "$PHP_POOL" "pm.min_spare_servers" "3"
set_ini_value "$PHP_POOL" "pm.max_spare_servers" "6"
set_ini_value "$PHP_POOL" "pm.max_requests" "350"
set_ini_value "$PHP_POOL" "request_terminate_timeout" "600"
set_ini_value "$PHP_POOL" "listen.backlog" "2048"
set_ini_value "$PHP_POOL" "request_slowlog_timeout" "15s"
set_ini_value "$PHP_POOL" "slowlog" "$PHP_FPM_SLOWLOG"

# ====================== php.ini ======================
echo "== Optimize php.ini =="
set_ini_value "$PHP_INI" "memory_limit" "384M"
set_ini_value "$PHP_INI" "max_execution_time" "600"
set_ini_value "$PHP_INI" "max_input_time" "600"
set_ini_value "$PHP_INI" "max_input_vars" "8000"
set_ini_value "$PHP_INI" "post_max_size" "128M"
set_ini_value "$PHP_INI" "upload_max_filesize" "128M"

# Thêm các thông số tốt
grep -q "^realpath_cache_size" "$PHP_INI" || echo "realpath_cache_size = 2048K" >> "$PHP_INI"
...
; JIT - Tracing (mức vừa cho 2GB RAM)
opcache.jit=tracing
opcache.jit_buffer_size=32M
EOF

# ====================== MariaDB ======================
echo "== Optimize MariaDB (2GB Profile) =="
cat > "$MYSQL_CNF" <<'EOF'
[mysqld]
...
echo "========================================"
echo "DONE! Optimization for 2GB RAM applied."
